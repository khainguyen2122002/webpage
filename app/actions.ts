...
async function getAdminUser() {
  ...
  if (user?.email !== adminEmail) throw new Error('Unauthorized')
...
export async function seedSampleData() {
  ...
  await supabase.from('center_info').upsert(centerInfo)
  ...
  for (const course of hrCourses) {
    await supabase.from('courses').upsert(course, { onConflict: 'slug' })
  }
